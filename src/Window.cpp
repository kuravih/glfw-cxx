...
namespace glfw
{
    ...
    {
        SetSizeLimits(minSize.x, minSize.y, maxSize.x, maxSize.y);
    }
    ...
    {
        return glfwSetWindowAttrib(m_window, attrib, value);
    }
    ...
    bool Window::GetEvents(Event &event)
    {
        ...
        std::queue<Event> *event_queue = static_cast<std::queue<Event> *>(glfwGetWindowUserPointer(m_window));
